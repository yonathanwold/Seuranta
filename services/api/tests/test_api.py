...
def test_health_keeps_databricks_sink_explicitly_disabled_without_config(client):
    response = client.get("/api/v1/health")
    assert response.status_code == 200
    databricks = response.json()["databricks"]
    assert databricks["status"] == "disabled"
    assert databricks["available"] is False
    assert databricks["reason"] == "credentials_or_warehouse_not_configured"
