...
describe('mock provider', () => {
  it('emits a complete anonymous demo snapshot and updates revisions', async () => {
    ...
    expect(snapshots[0].positions).toHaveLength(20); expect(snapshots[0].stateRevision).toBe(0)
    await vi.advanceTimersByTimeAsync(700); expect(snapshots.at(-1)?.stateRevision).toBe(1); expect(snapshots.at(-1)?.positions).toHaveLength(20)
