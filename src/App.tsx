...
function Navigation({ mode, onMode }: { mode: NormalizedMode; onMode: (mode: NormalizedMode) => void }) {
  return <aside className="navigation">
    ...
    <nav aria-label="Primary navigation">
      <div className="nav-item is-active" aria-current="page"><Target size={iconSize} /> <span>Live map</span></div>
    </nav>
    ...
    <div className="nav-mode">
      ...
      <div className="mode-options" role="group" aria-label="Data mode">
        {(['SIMULATION', 'LIVE'] as NormalizedMode[]).map((option) => <button key={option} className={mode === option ? 'is-selected' : ''} onClick={() => onMode(option)}>{modeCopy[option]}</button>)}
      </div>
    </div>
    <div className="nav-footer"><div className="profile"><div className="avatar">SA</div><div><strong>Seuranta Admin</strong><span>Operations</span></div><ChevronDown size={15} /></div></div>
  </aside>
}
...
function Explorer({ state, search, onSearch, selectedId, onSelect, showLayers, onToggle }: { state: ReturnType<typeof useAtlasStore.getState>['snapshot']; search: string; onSearch: (value: string) => void; selectedId: string | null; onSelect: (id: string) => void; showLayers: { entities: boolean; anchors: boolean; labels: boolean; confidence: boolean; zones: boolean }; onToggle: (key: keyof typeof showLayers) => void }) {
  ...
  return <aside className="explorer">
    <div className="explorer-tabs"><span className="is-active">Entities</span><button onClick={() => layerSectionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' })}>Map layers</button></div>
    ...
    <section className="layer-section" ref={layerSectionRef}><div className="section-heading"><span><Layers3 size={15} /> Map layers</span></div>
...
function DetailPanel({ state, selectedId, onClose, onSelect }: { state: ReturnType<typeof useAtlasStore.getState>['snapshot']; selectedId: string | null; onClose: () => void; onSelect: (id: string) => void }) {
  ...
  return <aside className="detail-panel"><div className="detail-heading"><div className="detail-title"><StatusDot status="online" /><div><h2>{selected ? titleForSession(selected.sessionId) : 'Select an entity'}</h2><p>{selected ? `Anonymous device · ${selected.sessionId.slice(-5).toUpperCase()}` : 'Choose a device or anchor on the map'}</p></div></div>{selected && <button className="icon-button" onClick={onClose} aria-label="Close details"><X size={18} /></button>}</div>{selected ? <div className="detail-body"><div className="detail-tabs"><span className="is-active">Details</span></div><div className="status-callout"><StatusDot status="online" /><span>Active ({modeCopy[selected.mode].toLowerCase()})</span></div><dl className="detail-list"><div><dt>Confidence</dt><dd>{Math.round(selected.confidence * 100)}%</dd></div><div><dt>Estimated accuracy</dt><dd>± {selected.accuracyRadiusM.toFixed(1)} m</dd></div><div><dt>Current zone</dt><dd>{zoneLabel(selected.zoneId)}</dd></div><div><dt>Coordinates (local)</dt><dd>X {selected.xM.toFixed(1)} m · Y {selected.yM.toFixed(1)} m</dd></div><div><dt>Last seen</dt><dd>{formatAge(selected.calculatedAt)}</dd></div></dl><div className="metric-line"><span>Signal confidence</span><strong>{Math.round(selected.confidence * 100)}%</strong><i><b style={{ width: `${selected.confidence * 100}%` }} /></i></div><h3>Anchors used ({selected.anchorsUsed.length})</h3><div className="anchor-coverage">{selected.anchorsUsed.map((anchorId) => <button key={anchorId} onClick={() => onSelect(anchorId)}><span className="anchor-glyph" /><span>Anchor {anchorId.replace('a-', '')}</span><StatusDot status={state.nodes.find((node) => node.anchorId === anchorId)?.status ?? 'online'} /><BarChart3 size={14} /></button>)}</div><h3>Recent activity</h3><ActivityFeed state={state} selected={selected} /></div> : <div className="detail-empty"><Sparkles size={22} /><p>Click an anonymous device or anchor to inspect live telemetry.</p></div>}</aside>
}
